...
namespace AwesomeTechnologies
{
    ...
    public class BillboardAtlasRenderer : MonoBehaviour
    {
        ...
        public static Texture2D GetDiffuseTexture(Material material)
        {
            if (material.HasProperty("_MainTex")) return (Texture2D)material.GetTexture("_MainTex");
            if (material.HasProperty("_BaseColorMap")) return (Texture2D)material.GetTexture("_BaseColorMap");
            if (material.HasProperty("_TrunkBaseColorMap")) return (Texture2D)material.GetTexture("_TrunkBaseColorMap");
            return null;
        }
        ...
        public static void SetReplacementShader(GameObject go, Shader replacementShader, bool generateAlpha)
        {
            ...
            for (int i = 0; i < renderers.Length; i++)
            {
                ...
                for (int j = 0; j < materials.Length; j++)
                {
                    ...
                    {
                        Texture2D diffuseTexture = GetDiffuseTexture(renderers[i].sharedMaterials[j]);
